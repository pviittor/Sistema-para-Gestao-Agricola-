...
import TituloPagarModal from '@/components/TituloPagarModal.vue'
import BaixaParcelaModal from '@/components/BaixaParcelaModal.vue'
import { tituloPagarService } from '@/services/tituloPagarService'
...
const editingItem = ref<TituloPagar | null>(null)
const selectedParcela = ref<ParcelaTituloPagar | null>(null)
const items = ref<TituloPagar[]>([])
const isLoading = ref(false)
const expandedRows = ref<Set<number>>(new Set())
const parcelasMap = ref<Map<number, ParcelaTituloPagar[]>>(new Map())
const loadingParcelas = ref<Set<number>>(new Set())

// KPIs
const kpis = ref<TituloPagarKpis>({
  totalAberto: 0,
  totalVencido: 0,
...
const selectedFornecedor = ref<number | ''>('')
const selectedPortador = ref<number | ''>('')
const selectedProdutor = ref<number | ''>('')

// Lookup options
const fornecedoresOptions = ref<Array<{ value: number; label: string }>>([])
...
const fetchPessoasLookups = async () => {
  try {
    ...
    fornecedoresOptions.value = pessoas.filter((p) => p.fornecedor_pessoa).map(toOption)
...
const agingChipClass = (parcela: ParcelaTituloPagar) => {
...
const agingLabel = (parcela: ParcelaTituloPagar) => {
...
const hasActiveFilters = computed(() =>
  ...
  || selectedFornecedor.value || selectedPortador.value || selectedProdutor.value
)
...
const fetchItems = async () => {
  ...
  try {
    ...
    if (safraStore.selectedSafraId) {
      result = await tituloPagarService.getBySafra(safraStore.selectedSafraId, currentPage.value, itemsPerPage.value)
    } else {
      ...
      if (selectedFornecedor.value) filtros.idFornecedor = selectedFornecedor.value
      ...
      result = await tituloPagarService.getAll(currentPage.value, itemsPerPage.value, filtros)
    }
    ...
    console.error(error)
    toast.error('Erro ao carregar títulos a pagar')
  } finally {
...
const fetchKpis = async () => {
  try {
    ...
    const result = await tituloPagarService.getKpis(filtros)
    kpis.value = result as TituloPagarKpis
  } catch (error) {
    console.error(error)
  }
}
...
const fetchParcelas = async (tituloId: number) => {
  ...
  try {
    const result = await tituloPagarService.getParcelas(tituloId)
...
const handleEdit = (item: TituloPagar) => {
  editingItem.value = item
  isModalOpen.value = true
}

const handleDelete = async (id: number) => {
  if (confirm('Tem certeza que deseja excluir este título a pagar?')) {
    try {
      await tituloPagarService.delete(id)
      ...
      console.error(error)
      toast.error('Erro ao excluir título. Verifique se existem parcelas baixadas.')
    }
  }
}

const handleSave = async (data: TituloPagar) => {
  isSaving.value = true
  try {
    if (editingItem.value?.id) {
      await tituloPagarService.update(editingItem.value.id, data as any)
      toast.success('Título atualizado com sucesso!')
    } else {
      await tituloPagarService.create(data as any)
    ...
    toast.error('Erro ao salvar título a pagar.')
  } finally {
...
// Baixa handlers
const handleBaixar = (parcela: ParcelaTituloPagar) => {
...
const handleBaixaSave = async (data: any) => {
  ...
  try {
    await tituloPagarService.baixarParcela(data)
    toast.success('Baixa realizada com sucesso!')
    isBaixaModalOpen.value = false
    // Refresh parcelas for the titulo
    if (selectedParcela.value) {
      await fetchParcelas(selectedParcela.value.idTituloPagar)
    }
    await fetchItems()
    await fetchKpis()
  } catch {
    toast.error('Erro ao realizar baixa da parcela.')
  } finally {
  ...
  <div class="min-h-screen bg-gray-50 p-6 md:p-8 font-sans">
    ...
    <div class="max-w-7xl mx-auto space-y-8">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 class="text-4xl font-bold text-gray-900">Contas a Pagar</h1>
          <p class="text-gray-500 mt-1">Gerencie os títulos e parcelas a pagar da sua propriedade</p>
        </div>
        <button
          @click="handleNew"
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-gray-400">{{ kpis.quantidadeAberto }} títulos</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-red-100 p-6">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-gray-400">{{ kpis.quantidadeVencido }} títulos</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm text-gray-500">Baixado no Mês</p>
              <p class="text-xl font-bold text-gray-900">{{ formatCurrency(kpis.totalBaixadoMes) }}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="p-6 border-b border-gray-100 space-y-4">
          <div class="flex flex-col sm:flex-row justify-between gap-4">
            <div class="flex items-center gap-2">
              <h2 class="text-lg font-bold text-gray-900">Títulos a Pagar</h2>
          ...
          <div v-if="showFilters" class="flex flex-wrap items-center gap-4 pt-2">
            ...
            <!-- Fornecedor -->
            <div class="flex items-center gap-2">
              <span class="text-sm text-gray-500 font-medium">Fornecedor:</span>
              <select
              ...
              >
                ...
                <option v-for="opt in fornecedoresOptions" :key="opt.value" :value="opt.value">
                  {{ opt.label }}
                </option>
              </select>
            </div>
          ...
          <table class="w-full">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Nº Título</th>
                <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Fornecedor</th>
                <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Data Lançamento</th>
            ...
            <tbody class="divide-y divide-gray-100">
              <template v-for="item in items" :key="item.id">
                ...
                >
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {{ item.fornecedor?.nomerazao_pessoa || '-' }}
                  </td>
              ...
              <tr v-if="items.length === 0">
                <td colspan="8" class="px-6 py-8 text-center text-gray-500 text-sm">
                  ...
                  <span v-else>Nenhum título a pagar encontrado.</span>
                </td>
              </tr>
            </tbody>
          </table>
