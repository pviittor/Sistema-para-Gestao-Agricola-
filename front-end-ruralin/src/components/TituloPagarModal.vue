...
const props = defineProps<{
  isOpen: boolean
  initialData?: TituloPagar | null
  loading?: boolean
}>()

// Lookup options (self-loaded)
const fornecedoresOptions = ref<OptionItem[]>([])
...
const fetchLookups = async () => {
  ...
  try {
    ...
    fornecedoresOptions.value = pessoasData.filter((p) => p.fornecedor_pessoa).map(toOption)
...
const formData = ref({
  numeroTitulo: '',
  idFornecedor: null as number | null,
...
const modalTitle = computed(() =>
  isEditing.value ? 'Editar Titulo a Pagar' : 'Novo Titulo a Pagar'
)
    ...
    if (newVal) {
      ...
      if (props.initialData) {
        formData.value = {
          ...
          idFornecedor: props.initialData.idFornecedor ?? null,
...
const handleSave = () => {
  ...
  if (!formData.value.idFornecedor) {
    ...
    <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
      ...
      <div class="flex items-center justify-between p-6 border-b border-gray-100">
        <div>
          ...
          <p class="text-sm text-gray-500 mt-1">Preencha os dados do titulo a pagar</p>
        </div>
        <button
          @click="$emit('close')"
