/**
 * Testes unitários para MovimentoFinanceiroTituloReceberRepository
 * 
 * Estes testes verificam os métodos customizados do MovimentoFinanceiroTituloReceberRepository
 * além dos métodos herdados do BaseRepository.
 */

import { Model, ModelStatic } from 'sequelize';
import { MovimentoFinanceiroTituloReceberRepository } from './MovimentoFinanceiroTituloReceberRepository';
import MovimentoFinanceiroTituloReceber from '../../models/MovimentoFinanceiroTituloReceber';
import { ICacheService } from '../../core/cache/ICacheService';
import { ITenantService } from '../../core/tenant/ITenantService';

jest.mock('../../models/MovimentoFinanceiroTituloReceber', () => {
  return {
    __esModule: true,
    default: jest.fn(),
  };
...
describe('MovimentoFinanceiroTituloReceberRepository', () => {
  let repository: MovimentoFinanceiroTituloReceberRepository;
  let mockModel: ModelStatic<MovimentoFinanceiroTituloReceber>;
  ...
  beforeEach(() => {
    ...
    mockModel = {
      ...
      create: jest.fn(),
      name: 'MovimentoFinanceiroTituloReceber',
    } as any;

    (MovimentoFinanceiroTituloReceber as any).findAll = mockFindAll;
    (MovimentoFinanceiroTituloReceber as any).sum = mockSum;

    mockTenantService.getTenantId.mockReturnValue(1);

    repository = new MovimentoFinanceiroTituloReceberRepository(mockCacheService, mockTenantService);
  ...
  describe('findByParcela', () => {
    it('deve retornar movimentos quando encontrados por parcela', async () => {
      const mockMovimentos = [
        { id: 1, idParcelaTituloReceber: 1 },
      ] as MovimentoFinanceiroTituloReceber[];
      ...
      expect(mockFindAll).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ idParcelaTituloReceber: 1 }),
        })
      );
    });

    it('deve retornar do cache quando disponível', async () => {
      const mockMovimentos = [{ id: 1 }] as MovimentoFinanceiroTituloReceber[];
