/**
 * Testes unitários para RateioCentroCustoTituloPagarRepository
 * 
 * Estes testes verificam os métodos customizados do RateioCentroCustoTituloPagarRepository
 * além dos métodos herdados do BaseRepository.
 */

import { Model, ModelStatic } from 'sequelize';
import { RateioCentroCustoTituloPagarRepository } from './RateioCentroCustoTituloPagarRepository';
import RateioCentroCustoTituloPagar from '../../models/RateioCentroCustoTituloPagar';
import { ICacheService } from '../../core/cache/ICacheService';
import { ITenantService } from '../../core/tenant/ITenantService';

jest.mock('../../models/RateioCentroCustoTituloPagar', () => {
  return {
    __esModule: true,
    default: jest.fn(),
  };
...
describe('RateioCentroCustoTituloPagarRepository', () => {
  let repository: RateioCentroCustoTituloPagarRepository;
  let mockModel: ModelStatic<RateioCentroCustoTituloPagar>;
  ...
  beforeEach(() => {
    ...
    mockModel = {
      ...
      create: jest.fn(),
      name: 'RateioCentroCustoTituloPagar',
    } as any;

    (RateioCentroCustoTituloPagar as any).findAll = mockFindAll;

    mockTenantService.getTenantId.mockReturnValue(1);

    repository = new RateioCentroCustoTituloPagarRepository(mockCacheService, mockTenantService);
  ...
  describe('findByTituloPagar', () => {
    it('deve retornar rateios quando encontrados por título', async () => {
      const mockRateios = [
        { id: 1, idTituloPagar: 1, idCentroCusto: 1 },
    ...
    it('deve retornar do cache quando disponível', async () => {
      const mockRateios = [{ id: 1 }] as RateioCentroCustoTituloPagar[];
