...
/**
 * Interface para atributos da entidade ParcelaTituloPagar
 */
interface ParcelaTituloPagarAttributes {
  id: number;
  tenantId: number;
 ...
 */
interface ParcelaTituloPagarCreationAttributes extends Optional<ParcelaTituloPagarAttributes, 'id' | 'datecreation' | 'valorParcelaMoedaOriginal' | 'valorParcelaMoedaPadrao' | 'taxaJuros' | 'valorJuros' | 'valorCorrecao' | 'taxaMulta' | 'valorMulta' | 'valorDesconto' | 'valorTotal' | 'valorPago' | 'valorSaldo' | 'dataBaixa' | 'valorBaixa' | 'idConta' | 'numeroTotalParcelas' | 'observacao' | 'status'> {}

/**
 * Modelo Sequelize para a entidade ParcelaTituloPagar
 * 
 * Representa uma parcela individual de um título a pagar.
 */
class ParcelaTituloPagar
  extends Model<ParcelaTituloPagarAttributes, ParcelaTituloPagarCreationAttributes>
  implements ParcelaTituloPagarAttributes
{
  public id!: number;
...
ParcelaTituloPagar.init(
  ...
  {
    sequelize,
    tableName: 'C020_parcelaTituloPagar',
    timestamps: false,
    underscored: false,
    indexes: [
      {
...
// Setup associations (both sides defined in child file per master-detail convention)
TituloPagar.hasMany(ParcelaTituloPagar, { foreignKey: 'idTituloPagar', as: 'parcelas' });
ParcelaTituloPagar.belongsTo(TituloPagar, { foreignKey: 'idTituloPagar', as: 'tituloPagar' });
ParcelaTituloPagar.belongsTo(Usuario, { foreignKey: 'usercreation', as: 'usuarioCriador' });
ParcelaTituloPagar.belongsTo(Conta, { foreignKey: 'idConta', as: 'conta' });

export default ParcelaTituloPagar;
