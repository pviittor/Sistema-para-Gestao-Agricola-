...
/**
 * CreateTituloPagarDto - DTO para criação de título a pagar
 * 
 * DTO usado no endpoint de criação de título a pagar com todas as validações necessárias.
 */
export class CreateTituloPagarDto extends CreateDto {
  /**
   * ID do fornecedor (pessoa marcada como fornecedor)
   */
  ...
  @Validate(IsExists, ['Pessoa', 'id_pessoa'], { message: 'Fornecedor não encontrado' })
  idFornecedor!: number;
  ...
  @IsUnique('TituloPagar', 'numeroTitulo', undefined, { message: 'Já existe um título com este número para este tenant' })
  numeroTitulo!: string;
   ...
   */
  @IsOptional()
  @IsEnum(StatusTituloPagar, { message: 'Status deve ser ABERTO, PARCIAL, BAIXADO ou CANCELADO' })
  status?: StatusTituloPagar;

  /**
   * Tipo do documento de origem (nota_fiscal_entrada, pedido_compra, emprestimo, manual)
   */
  @IsOptional()
  @IsString({ message: 'Tipo de origem deve ser uma string' })
  origemTipo?: string;

  /**
   * ID do documento de origem (polimórfico)
   */
  @IsOptional()
  @IsInt({ message: 'ID de origem deve ser um número inteiro' })
  @Min(1, { message: 'ID de origem deve ser maior que zero' })
  origemId?: number;
}
