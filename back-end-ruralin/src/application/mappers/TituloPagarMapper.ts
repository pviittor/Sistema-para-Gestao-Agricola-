/**
 * TituloPagarMapper - Mapper para entidade TituloPagar
 * 
 * Responsável por converter entre DTOs e entidades do domínio para TituloPagar.
 * 
 * Regras importantes:
 ...
 * @example
 * ```typescript
 * const mapper = new TituloPagarMapper();
 * 
 * // Converter DTO para entidade
 * const entity = await mapper.toEntity(createDto);
 * 
 * // Converter entidade para DTO
 * const dto = mapper.toDto(tituloPagar);
 * ```
 */
...
/**
 * Mapper para entidade TituloPagar
 * 
 ...
 */
@Injectable()
export class TituloPagarMapper implements IMapper<TituloPagar, TituloPagarResponseDto, CreateTituloPagarDto, UpdateTituloPagarDto> {
  /**
   ...
   */
  async toEntity(dto: CreateTituloPagarDto | UpdateTituloPagarDto): Promise<Partial<TituloPagar>> {
    const entity: any = {};

    // Campos de relacionamento (FKs)
    if ('idFornecedor' in dto && dto.idFornecedor !== undefined) entity.idFornecedor = dto.idFornecedor;
   ...
   */
  toDto(entity: TituloPagar): TituloPagarResponseDto {
    const dto: TituloPagarResponseDto = {
      id: entity.id,
      ...
      tipoGeracao: (entity as any).tipoGeracao || 'MANUAL',
      dataPrimeiraParcela: this.formatDate((entity as any).dataPrimeiraParcela),
      intervaloParcelasDias: (entity as any).intervaloParcelasDias,
      modeloJuros: (entity as any).modeloJuros,
      taxaJurosAm: (entity as any).taxaJurosAm,
    ...
    if (entity.fornecedor) {
      dto.fornecedor = {
        id_pessoa: entity.fornecedor.id_pessoa,
        nomerazao_pessoa: entity.fornecedor.nomerazao_pessoa,
        cpfcnpj_pessoa: entity.fornecedor.cpfcnpj_pessoa,
      };
    }
